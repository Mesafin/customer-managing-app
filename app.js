...
app.put("/update", (req, res) => {
  ...
  connection.query(updateCustomerQuery, [newName, id], (err) => {
    if (err) {
      console.error(err);
      return res.status(500).send("Error updating customer name.");
    }

    connection.query(updateAddressQuery, [newAddress, id], (err) => {
      if (err) {
        console.error(err);
        return res.status(500).send("Error updating address.");
      }

      connection.query(updateCompanyQuery, [newCompany, id], (err) => {
        if (err) {
          console.error(err);
